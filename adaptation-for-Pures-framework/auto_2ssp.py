...
"""
python3 adaptation-for-Pures-framework/auto_2ssp.py --stage s1 --s1-sparsity 0.05 --load-cifar --dataset cifar100 --cifar-train-pct 0.4 --cifar-test-pct 0.2 --calib-per-class 2 --eval-batches 5 --use-srp-checkpoint --fw-export-prefix adaptation-for-Pures-framework/artifacts/$(date +%Y%m%d-%H%M%S)/framework

python3 adaptation-for-Pures-framework/auto_2ssp.py --stage s2 --s2-sparsity 0.05 --force-depth-blocks 1 --load-cifar --dataset cifar100 --cifar-train-pct 0.4 --cifar-test-pct 0.2 --calib-per-class 2 --eval-batches 5 --use-srp-checkpoint --fw-export-prefix adaptation-for-Pures-framework/artifacts/$(date +%Y%m%d-%H%M%S)/framework

python3 adaptation-for-Pures-framework/auto_2ssp.py --stage both --target 0.15 --force-depth-blocks 1 --load-cifar --dataset cifar100 --cifar-train-pct 0.4 --cifar-test-pct 0.2 --calib-per-class 2 --eval-batches 5 --use-srp-checkpoint --fw-export-prefix adaptation-for-Pures-framework/artifacts/$(date +%Y%m%d-%H%M%S)/framework

python3 -m pip install -r adaptation-for-Pures-framework/requirements.txt. 
--fw-export-prefix adaptation-for-Pures-framework/artifacts/$(date +%Y%m%d-%H%M%S)/framework"""
...
# Local imports
import sys
ROOT = Path(__file__).resolve().parent  # self-contained package root (contains local src/)
...
PRUNING_DIR = ROOT  # mask_conjunction.py will be placed here
if str(PRUNING_DIR) not in sys.path:
    sys.path.insert(0, str(PRUNING_DIR))
from mask_conjunction import Auto2SSPInterface

# ======== Framework export helpers (scores + masks) ========
# We follow the format requested by the framework developer.
import json as _json

# Lightweight logging helpers for step/stage clarity
...
def export_scores_and_masks(prefix: str,
    ...
    with open(prefix + "_scores.json", "w") as f:
        _json.dump(scores_json, f, indent=2)
...
def build_framework_exports(prefix: str,
                            model,
                            mlp_imp_list,
                            att_imp_tensor,
                            ffn_masks_list,
                            pruned_attn_block_indices):
    """Build dictionaries for scores and masks in the framework format and dump to files.

    - mlp_imp_list: List[Tensor (d_int)] per block
    - att_imp_tensor: Tensor[B] with per-block attention importance
    - ffn_masks_list: Optional[List[List[int]]] per block (1=prune, 0=keep)
    - pruned_attn_block_indices: List[int] of blocks where attention was removed (Stage-2)
    """
    from src.vit_pruning import _get_encoder  # reuse anatomy helper
    ...
    hidden = getattr(getattr(model, "config", None), "hidden_size", None) or 768
    num_heads = getattr(getattr(model, "config", None), "num_attention_heads", None) or 12

    # Scores
    ffn_imp = {}
    for l, vec in enumerate(mlp_imp_list or []):
        try:
    ...
    # If att importance unavailable, fill zeros
    if att_imp_tensor is not None:
        try:
            att_vals = att_imp_tensor.detach().cpu().tolist()
        except Exception:
            att_vals = list(att_imp_tensor)
        if len(att_vals) < B:
            att_vals = (att_vals + [0.0] * B)[:B]
    else:
        att_vals = [0.0] * B

    head_imp = {}
    for l in range(B):
        base = float(att_vals[l] if l < len(att_vals) else 0.0)
    ...
    for l in range(B):
        ...
        for d in range(hidden):
            qkv_dim_imp[(l, d)] = base

    # Masks (lists per layer)
    ffn_mask = {}
    if ffn_masks_list is not None and len(ffn_masks_list) == B:
        for l, m in enumerate(ffn_masks_list):
            ffn_mask[l] = m
    else:
        # fallback: zeros with appropriate size per layer derived from mlp_imp_list
        for l in range(B):
            d_int = len(mlp_imp_list[l]) if (mlp_imp_list and l < len(mlp_imp_list)) else hidden * 4
            ffn_mask[l] = [0] * int(d_int)

    pruned_set = set(pruned_attn_block_indices or [])
    head_mask = {}
    for l in range(B):
        if l in pruned_set:
            head_mask[l] = [1] * num_heads
        else:
            head_mask[l] = [0] * num_heads

    qkv_dim_mask = {}
    for l in range(B):
        if l in pruned_set:
            qkv_dim_mask[l] = [1] * hidden
        else:
            qkv_dim_mask[l] = [0] * hidden

    export_scores_and_masks(prefix, ffn_imp, head_imp, qkv_dim_imp, ffn_mask, head_mask, qkv_dim_mask)
    return {
        "ffn_imp": ffn_imp,
        "head_imp": head_imp,
        "qkv_dim_imp": qkv_dim_imp,
        "ffn_mask": ffn_mask,
        "head_mask": head_mask,
        "qkv_dim_mask": qkv_dim_mask,
    }
...
def run(args):
    ...
    if getattr(args, "use_srp_checkpoint", False):
        # Load SRP timm checkpoint and convert weights to HF model (no tensorflow required)
        srp_ds = getattr(args, "srp_dataset", "cifar100")
        models_dir = args.srp_models_dir
        index_csv = args.srp_index_csv
        timm_model, srp_meta = _load_srp_model(
        ...
        # adjust classifier to dataset (default: CIFAR variants)
        num_classes = 100 if srp_ds.lower() == "cifar100" else 37
        hidden = getattr(model.config, "hidden_size", 768)
        model.classifier = torch.nn.Linear(hidden, num_classes, device=device)
    ...
    maxb = args.baseline_eval_batches if args.baseline_eval_batches is not None else args.eval_batches
    if test_loader is not None and (maxb is None or maxb > 0):
        acc_baseline = evaluate_top1(model, test_loader, device=device, max_batches=maxb, progress=True)
    else:
        acc_baseline = None
    _log_step(f"Baseline computed: params={params_before}, latency_ms={round(latency_baseline*1000,2)}, acc={acc_baseline}")
    ...
    importance_mode_cli = (args.depth_importance if args.depth_importance in ("copy", "heuristic") else "heuristic")
    # Decide final importance mode with safe defaults on MPS
    imp_mode = "heuristic" if args.stage == "s1" else importance_mode_cli
    if args.stage == "s2" and imp_mode == "copy" and (str(device).startswith("mps")) and not getattr(args, "force_copy_eval", False):
        _log_step("[ATT-IMPORTANCE] MPS detected: switching depth importance from 'copy' to 'heuristic' to avoid long copy-eval. Pass --force-copy-eval to override.")
        imp_mode = "heuristic"
    ...
    _log_step(f"Computing importances: stage={args.stage}, mode={imp_mode}")
    mlp_imp = iface._compute_mlp_importance() if (args.stage in ("both", "s1") and args.s1_importance == "act") else None
    att_imp = iface._compute_att_depth_importance() if args.stage in ("both", "s2") else None
    _log_step(f"Importances ready: mlp_imp={'yes' if mlp_imp is not None else 'no'}, att_imp={'yes' if att_imp is not None else 'no'}")
    ...
    if args.stage in ("both", "s1"):
        ...
        _log_stage("STAGE-1", f"Starting width pruning: min_remaining={args.min_remaining}, first_block_n_prune={n_to_prune_per_block[0] if len(n_to_prune_per_block)>0 else 0}")
        s1_res = prune_vit_mlp_width(
            ...
            progress=True,
            collect_masks=True,
            precomputed_importance=([x.to(torch.float32) for x in mlp_imp] if mlp_imp is not None else None),
        )
    ...
    if test_loader is not None:
        ...
        acc_stage1 = None
    print(f"[STAGE-1 DONE] params={params_after_stage1}, latency_ms={round(latency_stage1*1000,2)}, acc={acc_stage1}")
    ...
    if args.stage in ("both", "s2"):
        ...
        _log_stage("STAGE-2", f"Starting depth pruning: num_to_prune={num_to_prune if num_to_prune is not None else int(round(depth_fraction * B))}, B={B}, mode={args.depth_importance}")
    ...
    if test_loader is not None:
        ...
        acc_stage2 = None
    print(f"[STAGE-2 DONE] params={params_after_stage2}, latency_ms={round(latency_stage2*1000,2)}, acc={acc_stage2}, pruned_blocks={pruned_indices}")
    ...
    # Гарантированно не сохраняем attention_pruned_indices.json (и удаляем, если появился)
    attn_indices_path = artifacts_dir / "attention_pruned_indices.json"
    if attn_indices_path.exists():
        try:
            attn_indices_path.unlink()
            print(f"[CLEANUP] Removed stale {attn_indices_path}")
        except Exception as e:
            print(f"[CLEANUP] Could not remove {attn_indices_path}: {e}")
    # больше не используем этот файл вообще
    attn_indices_path = None

    # Опционально сохраняем FFN masks, если они есть (и делаем путь доступным)
    ffn_masks_path = None
    if ffn_masks is not None:
        try:
            ...
            masks_serializable = [_to_int_list(m) for m in ffn_masks]
            ffn_masks_path = artifacts_dir / "ffn_prune_masks.json"
            with open(ffn_masks_path, "w", encoding="utf-8") as f:
                json.dump({"ffn_masks": masks_serializable}, f, indent=2)
            print(f"[ARTIFACT] FFN prune masks saved to: {ffn_masks_path}")
        except Exception as e:
            print(f"[WARN] Could not save FFN masks: {e}")
            ffn_masks_path = None

    artifacts: Dict[str, Any] = {
        "pruned_block_indices": pruned_indices,
    }
    if ffn_masks_path is not None:
        artifacts["ffn_prune_masks_path"] = str(ffn_masks_path)
    # attention_pruned_indices.json больше не сохраняем и не добавляем в артефакты
    if pruned_model_dir is not None:
        artifacts["pruned_model_dir"] = pruned_model_dir

    # Export framework-compatible JSONs if requested
    if getattr(args, "fw_export_prefix", None):
        try:
            export_prefix = args.fw_export_prefix
            build_framework_exports(export_prefix, model, mlp_imp, att_imp, ffn_masks, pruned_indices)
            print(f"[FRAMEWORK] Exported to: {export_prefix}_scores.json and {export_prefix}_masks.json")
        except Exception as e:
            print(f"[FRAMEWORK] Export failed: {e}")
...
def build_argparser():
    ...
    p.add_argument("--baseline-eval-batches", type=int, default=None, help="Override batches used for baseline accuracy eval; set 0 to skip baseline eval")
    p.add_argument("--s1-importance", type=str, default="act", choices=["act", "l1"], help="Importance for Stage-1 width: act (activation L2 over calibration) or l1 (weight L1, fast)")
    p.add_argument("--force-copy-eval", action="store_true", help="Force depth importance 'copy' on Stage-2 even on MPS (may be slow)")
    ...
    p.add_argument("--srp-index-csv", type=str, default=str(ROOT / "models" / "index.csv"), help="Path to SRP models/index.csv")
    p.add_argument("--srp-models-dir", type=str, default=str(ROOT / "models"), help="Directory containing SRP .npz checkpoints")
    ...
    p.add_argument("--fw-export-prefix", type=str, default=None, help="If set, export framework JSONs to <prefix>_scores.json and <prefix>_masks.json")
    return p
