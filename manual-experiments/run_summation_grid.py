...
- Перебрать все комбинации из 4 заданных файлов (все 1/2/3/4-сочетания, порядок не важен).
- Для каждой комбинации прогнать --prune в {5,10,15,...,50}.
- Вторую команду не менять.
...
- По возможности всё в один результирующий файл (results.csv). Если пишем дополнительные файлы (логи),
  то в имени отмечать комбинацию и уровень sparsity. Без временных меток.

Запуск (полный):
  python3 manual-experiments/run_summation_grid.py

Запуск (мини-прогон для проверки: только одиночки и prune=5,10):
  python3 manual-experiments/run_summation_grid.py --sizes 1 --prune-levels 5,10

Опции CLI:
  --sizes "1,2,3,4"       — какие размеры сочетаний брать (по умолчанию все 1..4)
  --prune-levels "5,10"   — перечисление уровней sparsity (если не задано — 5..50 шаг 5)
  --first-n-combos N      — ограничить число первых комбинаций (после сортировки) для быстрого теста

Выходные файлы:
  final-results-for-presentation/summation/results.csv          — сводка по всем запускам
  final-results-for-presentation/summation/logs/*.stdout.txt    — сырые логи второй стадии (по комбинации и prune)
  final-results-for-presentation/summation/logs/*.stderr.txt
...
AGG_SCRIPT = ROOT / "manual-experiments" / "aggregate_and_mask-summation.py"
APPLY_SCRIPT = ROOT / "experiments" / "vit_pruning" / "apply_mask_prune.py"
MASK_PATH = ROOT / "manual-experiments" / "mask.json"
...
DEFAULT_PRUNE_LEVELS: List[int] = list(range(5, 51, 5))

# Куда писать сводные результаты и логи (без временных меток)
OUT_DIR = ROOT / "final-results-for-presentation" / "summation"
...
def check_prerequisites() -> None:
    if not AGG_SCRIPT.exists():
        raise FileNotFoundError(f"Aggregate script not found: {AGG_SCRIPT}")
...
def build_apply_cmd() -> List[str]:
    # Команду не меняем вообще (как просил пользователь)
    return [
        sys.executable,
        str(APPLY_SCRIPT),
        "--mask",
...
def iterate_combinations(files: Sequence[Path], sizes: Set[int]) -> Iterable[Tuple[Path, ...]]:
    # Все сочетания выбранных размерностей, порядок не важен
...
def parse_sizes(arg: Optional[str]) -> Set[int]:
    ...
    for p in parts:
        ...
        if v not in (1, 2, 3, 4):
            raise argparse.ArgumentTypeError(f"Size must be in 1..4, got: {v}")
        vals.add(v)
    if not vals:
        vals = {1, 2, 3, 4}
    return vals
...
def build_argparser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Grid-раннер для суммирования нормализованных скорингов и применения маски.")
    ap.add_argument("--sizes", type=str, default=None, help="Какие размерности сочетаний использовать, например '1,2' или '1,2,3,4' (по умолчанию все).")
...
def main() -> None:
    ...
    for files_combo in combos:
        ...
        for prune in prune_levels:
            ...
            if rc1 != 0:
                print(f"[ERROR] aggregate_and_mask-summation failed (rc={rc1}). Перехожу к следующему прогону.")
                ...
                append_csv_row(
                    CSV_PATH,
                    {
                        "methods": ckey,
                        "prune": prune,
                        "status": f"aggregate_failed_rc_{rc1}",
                    },
                )
                # Сохраним stderr/stdout для отладки
                (LOGS_DIR / f"{ckey_safe}_p{prune}.aggregate.stderr.txt").write_text(err1 or "", encoding="utf-8")
                (LOGS_DIR / f"{ckey_safe}_p{prune}.aggregate.stdout.txt").write_text(out1 or "", encoding="utf-8")
                continue
            ...
            # Пишем сырые логи второй стадии по просьбе пользователя
